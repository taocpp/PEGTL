...
Here, *equivalence* is *only* regarding which inputs are matched.
The equivalent rule is to be understood as shortest documentation of the matching behaviour of a rule, but *not* how it is implemented and how it will behave towards Actions and Control.
...
* Implemented as `list< R, S, P >` public `internal::list< R, internal::pad< S, P > >` using `internal::seq< R, internal::star< internal::pad< S, P >, R > >`.
...
* Implemented as `minus< M, S >` public `internal::minus< M, S >` using `internal::rematch< M, internal::not_at< S, internal::eof > >`.
