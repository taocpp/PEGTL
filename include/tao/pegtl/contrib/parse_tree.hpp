...
namespace TAO_PEGTL_NAMESPACE::parse_tree
{
   ...
   struct basic_node
   {
      ...
      template< tracking_mode P = tracking_mode::eager, typename Eol = eols::scan::lf_crlf >
      [[nodiscard]] memory_input< P, Eol > as_memory_input() const
      {
         assert( has_content() );
         return { m_begin.data, m_end.data, source, m_begin.line, m_begin.column, m_begin.byte };
      }
