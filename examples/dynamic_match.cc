...
namespace dynamic
{
   ...
   {
      template< pegtl::apply_mode A, template< typename ... > class Action, template< typename ... > class Control >
   ...
   {
      static void apply( const pegtl::input & in, std::string & long_literal_mark, const std::string & )
      {
   ...
   {
      static void apply( const pegtl::input & in, const std::string &, std::string & long_literal_body )
      {
...
int main( int argc, char * argv[] )
{
   if ( argc > 1 ) {
      ...
      pegtl::parse< dynamic::grammar, dynamic::action >( 1, argv, long_literal_mark, long_literal_body );
