...
namespace TAO_PEGTL_NAMESPACE
{
   ...
   void unit_test()
   {
      int state_r = 0;
      int state_s = 0;
      TAO_PEGTL_TEST_ASSERT( parse< apply0< test1::action_a, test1::action_b > >( text_view_input< ascii::lf >( "" ), state_r, state_s ) );
      TAO_PEGTL_TEST_ASSERT( state_r == 1 );
      TAO_PEGTL_TEST_ASSERT( state_s == 2 );
      TAO_PEGTL_TEST_ASSERT( parse< disable< apply< test1::action_a, test1::action_b > > >( text_view_input< ascii::lf >( "" ), state_r, state_s ) );
      TAO_PEGTL_TEST_ASSERT( state_r == 1 );
      TAO_PEGTL_TEST_ASSERT( state_s == 2 );

      bool state_b = false;
      TAO_PEGTL_TEST_ASSERT( !parse< apply0< test1::action2_a, test1::action2_b, test1::action2_c > >( text_view_input< ascii::lf >( "" ), state_b ) );
      TAO_PEGTL_TEST_ASSERT( state_b );
