...
namespace tao
{
   namespace TAOCPP_PEGTL_NAMESPACE
   {
      ...
      void unit_test()
      {
         int state_r = 0;
         int state_s = 0;
         parse_memory< must< apply0< test1::action_a, test1::action_b > > >( { "", __FILE__ }, state_r, state_s );
         TAOCPP_PEGTL_TEST_ASSERT( state_r == 1 );
         TAOCPP_PEGTL_TEST_ASSERT( state_s == 2 );
         parse_memory< must< disable< apply< test1::action_a, test1::action_b > > > >( { "", __FILE__ }, state_r, state_s );
         TAOCPP_PEGTL_TEST_ASSERT( state_r == 1 );
         TAOCPP_PEGTL_TEST_ASSERT( state_s == 2 );
