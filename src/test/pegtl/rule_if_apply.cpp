...
namespace tao
{
   namespace TAOCPP_PEGTL_NAMESPACE
   {
      ...
      void unit_test()
      {
         ...
         parse_memory< must< if_apply< one< '-' >, test1::action_a, test1::action_b > >, test1::action >( { "-", __FILE__ }, state_r, state_s );
         TAOCPP_PEGTL_TEST_ASSERT( test1::flag == 1 );
         TAOCPP_PEGTL_TEST_ASSERT( state_r == "-" );
         TAOCPP_PEGTL_TEST_ASSERT( state_s == "-*-" );
         parse_memory< must< disable< if_apply< one< '-' >, test1::action_a, test1::action_b > > >, test1::action >( { "-", __FILE__ }, state_r, state_s );
         TAOCPP_PEGTL_TEST_ASSERT( test1::flag == 1 );
         TAOCPP_PEGTL_TEST_ASSERT( state_r == "-" );
         TAOCPP_PEGTL_TEST_ASSERT( state_s == "-*-" );
