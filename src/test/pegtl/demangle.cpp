...
namespace TAO_PEGTL_NAMESPACE
{
   ...
   void unit_test()
   {
      ...
      test< double >( "d" );
      if( ns == "tao::pegtl" ) {
         test< seq< many< 42 >, eof > >( "N3tao5pegtl3seqIJNS0_4manyILj42EEENS0_3eofEEEE" );
      }
#elif defined( _MSC_VER ) && !defined( __clang__ )
      test< int >( "int" );
      test< double >( "double" );
      // in the Microsoft world, class and struct are not the same!
      test< seq< many< 42 >, eof > >( "struct " + ns + "::seq<struct " + ns + "::many<42>,struct " + ns + "::eof>" );
#else
      test< int >( "int" );
      test< double >( "double" );
      test< seq< many< 42 >, eof > >( ns + "::seq<" + ns + "::many<42>, " + ns + "::eof>" );
#endif
   }
