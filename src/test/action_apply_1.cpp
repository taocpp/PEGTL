...
namespace TAO_PEGTL_NAMESPACE
{
   ...
   void unit_test()
   {
      const auto result = parse< disable< test1::bar >, test_action >( text_view_input< scan::lf >( "baab" ) );
      ...
      parse< at< action< test_action, test1::bar > > >( text_view_input< scan::lf >( "baab" ) );

      TAO_PEGTL_TEST_ASSERT( applied.empty() );

      applied.clear();

      parse< test1::bar, test_action >( text_view_input< scan::lf >( "baab" ) );

      test1::test_result();

      applied.clear();

      parse< action< test_action, test1::bar > >( text_view_input< scan::lf >( "baab" ) );

      test1::test_result();

      applied.clear();

      parse< disable< enable< action< test_action, test1::bar > > > >( text_view_input< scan::lf >( "baab" ) );
