...
namespace TAO_PEGTL_NAMESPACE
{
   ...
   void test_common( As&&... as )  // NOLINT(readability-function-size)
   {
      ...
      internal::stream_input_base< Buffer > bc( std::forward< As >( as )..., st );

      const auto* start = bc.buffer_start();

      static_assert( std::is_same_v< char, typename Buffer::data_t > );
      static_assert( std::is_same_v< char, typename internal::stream_input_base< Buffer >::data_t > );
   ...
   void unit_test()
   {
      test_common< internal::alloc_buffer< char, internal::endless_reader > >( 100, 20 );
      test_common< internal::array_buffer< char, internal::endless_reader, 100, 20 > >();
   }
