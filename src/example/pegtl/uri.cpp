...
struct URI
{
   ...
   explicit URI( pegtl::argv_input< pegtl::eols::scan::lf_crlf >& );
};
...
URI::URI( pegtl::argv_input< pegtl::eols::scan::lf_crlf >& in )
{
...
int main( int argc, char** argv )
{
   try {
      for( int i = 1; i < argc; ++i ) {
         ...
         pegtl::argv_input< pegtl::eols::scan::lf_crlf > in( argv, i );
         const URI uri( in );
