...
namespace examples
{
   ...
   template< typename Rule > struct action : unescape_action< Rule > {};  // Inherit from json_unescape.hpp.
...
int main( int argc, char ** argv )
{
   if ( argc != 2 ) {
     std::cerr << "usage: " << argv[ 0 ] << " <json>";
      ...
      tao::TAOCPP_PEGTL_NAMESPACE::file_parser( argv[ 1 ] ).parse< examples::grammar, examples::action, examples::control >( state );
